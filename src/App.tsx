...
function App() {
  ...
  // Agent Wrapped state
  const [wrappedRange, setWrappedRange] = useState<WrappedRange>('today');
  const [wrappedProjectPath, setWrappedProjectPath] = useState<string | null>(null);
  const [wrappedData, setWrappedData] = useState<WrappedOut | null>(null);
  ...
  async function refreshWrapped(next?: { range?: WrappedRange; projectPath?: string | null }) {
    try {
      setWrappedErr(null);
      setWrappedLoading(true);
      const r = next?.range ?? wrappedRange;
      const p = next?.projectPath ?? wrappedProjectPath;
      const out = await fetchWrapped({ range: r, projectPath: p });
      setWrappedData(out);
      setWrappedProjects(out.projects ?? []);
      // If project selection is unset, default to the server-selected project.
      if (!p && out.card2?.project?.project_path) setWrappedProjectPath(out.card2.project.project_path);
    } catch (e: any) {
  ...
  const orbitVisible = useMemo(() => {
    ...
    // IMPORTANT: In production, integrations_status can briefly/mistakenly report
    // an IDE as disabled (e.g. legacy VS Code hooks that still emit events but
    // don't match our shim path heuristics). If we have live events for a family,
    // do not hide that avatar.
    const filtered = enabledSet
	  ? withoutDismissed.filter((k) => {
			const fam = String(byKey[k]?.agentFamily ?? '').toLowerCase();
			// Always show unknown families if they have events (best-effort).
			if (!isIntegrationKey(fam)) return true;
			// Show enabled integrations.
			if (enabledSet.has(fam)) return true;
			// Also show disabled integrations if we have live events for them.
			return Boolean(byKey[k]);
		})
      : withoutDismissed;
                ...
                >
                  ...
                  {wrappedData ? (
                    <WrappedCard
                      ...
                      selectedProjectPath={wrappedProjectPath}
                      onChangeRange={(r) => {
                        setWrappedRange(r);
                        void refreshWrapped({ range: r });
                      }}
                      onChangeProjectPath={(p) => {
                    ...
                    <div className="wrappedPanel">
                      ...
                      <div className="settingsSub">{wrappedLoading ? 'Loading…' : 'No data yet.'}</div>
                    </div>
                  )}
                </div>
              ) : null}
